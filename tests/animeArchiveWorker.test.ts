...
describe('anime archive worker safety boundaries', () => {
  ...
  it('fails closed when an episode exceeds the configured single-file Telegram ceiling', async () => {
    const state = harness({ candidates: [directCandidate(1)] });

    await runHarness(state, { archive: { maxUploadBytes: 4 }, waitFor: 'failure' });

    expect(mocks.normalize).not.toHaveBeenCalled();
    expect(mocks.send).not.toHaveBeenCalled();
    expect(state.reserveMedia).not.toHaveBeenCalled();
    expect(state.jobs.beginEpisodeDelivery).not.toHaveBeenCalled();
    expect(state.jobs.failEpisode).toHaveBeenCalledOnce();
    expect(state.jobs.failEpisode.mock.calls[0]?.[3]).toContain('single-file Telegram limit');
