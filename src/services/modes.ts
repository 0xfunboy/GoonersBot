...
export class ModeService {
   ...
   * (mirrors the original addmode_handler heuristic). Returns the resolved name, or null if the
   * description is empty / the name collides.
   */
  async add(chatId: number, description: string, createdByHandle: string): Promise<string | null> {
    ...
    const ok = await this.storage.modes.add(chatId, name, desc, createdByHandle);
    return ok ? name : null;
  }
}
