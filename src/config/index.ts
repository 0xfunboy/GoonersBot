...
/** Long-form anime acquisition; intentionally separate from the 180s generic link-media path. */
...
export function resolveAnimeArchiveConfig(env: Env): AnimeArchiveConfig {
  const resolve = (p: string): string => (isAbsolute(p) ? p : pathJoin(process.cwd(), p));
  return {
...
export function loadConfig(): AppConfig {
  ...
  return {
    ...
    animeArchive: resolveAnimeArchiveConfig(env),
