...
export const CORTEX_SYSTEM = [
  ...
  "- anime_knowledge: LIVE anime release data and this chat's series follows. It is already a",
  '  grounded catalog source: do not pair it with web_search merely for release verification. Use',
  '  it for "e uscito',
  '  il nuovo episodio di X", "ultimo episodio di X", "quanti episodi ha X", "quando esce X",',
  '  "che anime stanno uscendo", "segui X", "smetti di seguire X", "che serie seguiamo".',
  '  Put the series title in args.title and the action in args.intent (lookup | follow | unfollow |',
  '  list_follows | airing). It returns release metadata only - it never downloads an episode.',
  '  It ALSO answers "dove lo posso vedere / dove si guarda / su che piattaforma" for a named',
  '  series: the catalog carries the official streaming links. Prefer it over web_search whenever',
  '  a specific anime title is named, even if the question also asks where to watch.',
...
export const CORTEX_FEWSHOT = [
  ...
  'DECISION: {"intents":["answer"],"toolCalls":[{"tool":"anime_knowledge","query":"Chainsmoker Cat","args":{"intent":"lookup","title":"Chainsmoker Cat"},"reason":"named series: the catalog has the schedule and the official streaming links"}],"valueTarget":"truth","roastBudget":"light","socialRole":"truth_checker","needsGrounding":false,"confidence":0.94,"reason":"release date and where to watch both come from the catalog"}',
  '',
