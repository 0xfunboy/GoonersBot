...
export const CORTEX_SYSTEM = [
  ...
  "- anime_knowledge: anime metadata and this chat's series follows. Use it for title resolution,",
  '  episode metadata and follow state, but NEVER treat catalog provenance as a watch/download',
  '  source and NEVER invent or expose AniList/Jikan/Crunchyroll/other gateway links as availability.',
  '  For "e uscito il nuovo episodio di X", "ultimo episodio di X", "quanti episodi ha X",',
  '  "quando esce X", "che anime stanno uscendo", "segui X", "smetti di seguire X", "che serie',
  '  seguiamo", put the series title in args.title and the action in args.intent (lookup | follow |',
  '  unfollow | list_follows | airing). It never downloads an episode.',
  '  For "dove lo posso vedere / dove si guarda / me lo scarichi / rehostami" on a named anime,',
  '  still resolve the title with anime_knowledge when useful, but availability/rehost must come',
  '  only from the deterministic AnimeUnity/HentaiSaturn archive layer. Do not add web_search just',
  '  to find another streaming provider or gateway.',
...
export const CORTEX_FEWSHOT = [
  ...
  '',
  'MESSAGE (it): "quando esce il prossimo episodio di Chainsmoker Cat e dove lo posso vedere?"',
  'DECISION: {"intents":["answer"],"toolCalls":[{"tool":"anime_knowledge","query":"Chainsmoker Cat","args":{"intent":"lookup","title":"Chainsmoker Cat"},"reason":"resolve the named series and release metadata; watch availability is handled separately by the AnimeUnity/HentaiSaturn archive layer"}],"valueTarget":"truth","roastBudget":"light","socialRole":"truth_checker","needsGrounding":false,"confidence":0.94,"reason":"use catalog metadata for the answer and let the deterministic no-gateway archive layer own availability/rehost"}',
  '',
